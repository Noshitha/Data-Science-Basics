...
predict(reg,type='prediction')#predict(reg,type = "prediction")
...
predict(reg_log)
plot(log(Waist),AT)#predict(reg_log,interval="predict")
...
# Quadratic model
quad_mod <- lm(AT~Waist+Waist*Waist,data=wc.at)
summary(quad_mod)
waist_sq = Waist*Waist
# Quadratic model
qud_mod <- lm(AT~Waist+I(Waist^2),data=wc.at)
summary(qud_mod)
# Quadratic model
attach(wc.at)
qd_model <- lm(AT~Waist+waist_sq,data=wc.at)
summary(qd_model)
