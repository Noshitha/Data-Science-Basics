...
wcat=pd.read_csv("C:\\Users\\dell\\Desktop\\studies stuff\\data science\\Simple Linear Regression\\Pythoncode\\wc-at.csv")
wcat.columns

plt.hist(wcat.Waist)
plt.boxplot(wcat.Waist)
plt.boxplot(wcat.AT,0,"rs",1)
...
np.sqrt(np.mean((pred-wcat.AT)**2))
...
pred2
plt.scatter(x=wcat['Waist'],y=wcat['AT'],color='green');sns.lineplot(wcat['Waist'],pred2);plt.xlabel('WAIST');plt.ylabel('TISSUE')
plt.hist(wcat.AT-pred2)
plt.scatter(x=wcat.Waist,y = pred2)
...
pred3
pred3.corr(wcat.AT)
plt.scatter(x=wcat['Waist'],y=wcat['AT'],color='green');sns.lineplot(wcat.Waist,np.exp(pred_log),color='blue');plt.xlabel('WAIST');plt.ylabel('TISSUE')
...
plt.scatter(wcat.Waist,wcat.AT,c="b");sns.lineplot(wcat.Waist,pred_quad)
...
model1.fit(wcat.Waist.values.reshape(109,1),wcat.AT)

pred1 = model1.predict(wcat.Waist.values.reshape(109,1))
...
rmse3 = np.sqrt(np.mean(((pred3)-wcat.AT))**2) # 32.0507
model3.coef_
model3.intercept_
